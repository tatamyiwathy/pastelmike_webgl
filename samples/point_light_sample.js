...
function main() {
  ...
  class SphereRotator extends Animator {
    constructor(options = {}) {
      super();
      this.angle = 0;
      this.rotateDirection = options.rotate === 'clockwise' ? -1 : 1;
    }
    update(obj, deltaTime) {
      this.angle = (2 * Math.PI / 30) * deltaTime * this.rotateDirection;
  ...
  sphere_mesh2.animator = new SphereRotator({rotate: 'clockwise'});
